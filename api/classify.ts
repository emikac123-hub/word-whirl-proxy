...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const sys =
      'Classify ONE token. Return JSON: {"valid": boolean, "pos": ["noun"|"verb"|"adjective"|"adverb"|"properNoun"], "base": string}. "valid" must be true ONLY if the word is PRIMARILY used as that part of speech in everyday language (not occasionally).' +
      'For example: "running" is techincally a noun because it is a gerund. But it should not be considered "valid" as a noun, becuase that is not its primary usage.'

    const user = `Locale: ${locale || 'en'}\nWord: "${word}"\nRequiredPOS: ${requiredPOS || 'none'}`;
    ...
    try {
      ...
      return res.status(502).json({ error: 'Upstream error (non-JSON from OpenAI)' });
    }
